...
def vis_parsing_maps(img, parsing_anno, stride, save_anno_path=None, save_vis_path=None):
    ...
    if save_vis_path is not None:
        ...
        vis_im = cv2.resize(vis_im, (1920, 1080), interpolation=cv2.INTER_LINEAR)
        cv2.imwrite(save_vis_path, vis_im)


def parsing(img_path, output):
...
if __name__ == '__main__':
    path_list = os.listdir("D:/mnt/meta_process/stimuli")
    for path in path_list:
        img_path = os.path.join("D:/mnt/meta_process/stimuli", path)
        output = "./result"
        parsing(img_path, output)
